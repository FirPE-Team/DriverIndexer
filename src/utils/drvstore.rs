...
impl std::fmt::Display for DrvstoreError {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            DrvstoreError::Win32Error(code) => write!(f, "Win32 Error: {}", code),
            DrvstoreError::LibraryError(err) => write!(f, "Library Error: {}", err),
            DrvstoreError::Message(msg) => write!(f, "{}", msg),
        }
    }
}

impl std::error::Error for DrvstoreError {
    fn source(&self) -> Option<&(dyn std::error::Error + 'static)> {
        match self {
            DrvstoreError::LibraryError(err) => Some(err),
            _ => None,
        }
    }
}

impl From<libloading::Error> for DrvstoreError {
    fn from(err: libloading::Error) -> Self {
        DrvstoreError::LibraryError(err)
    }
}
...
impl DriverStore {
    ...
    ///
    /// # 参数
    /// - `path` - 可选的 drvstore.dll 路径，默认值为 `drvstore.dll`
    ///
    /// # 返回值
    /// - `Ok(DriverStore)`: 成功加载并解析函数
    /// - `Err(...)`：加载失败则返回错误信息
    pub fn new(path: Option<&Path>) -> Result<Self, libloading::Error> {
        let lib = unsafe { Library::new(path.unwrap_or(Path::new("drvstore.dll"))) }?;
        unsafe {
            Ok(Self {
    ...
    pub fn open_store(
        ...
        system_root: &Path,
        system_drive: &Path,
    ) -> Result<Handle, DrvstoreError> {
        let root = to_wide(system_root.as_os_str());
        let drv = to_wide(system_drive.as_os_str());
        let handle = unsafe { (self.openStore)(root.as_ptr(), drv.as_ptr(), 0, 0) };
        if handle != 0 {
            Ok(handle)
        } else {
            unsafe { Err(DrvstoreError::Win32Error(GetLastError().0)) }
        }
    }
    ...
    pub fn close_store(&self, handle: Handle) -> Result<(), DrvstoreError> {
        if !unsafe { (self.closeStore)(handle) } {
            unsafe { Err(DrvstoreError::Win32Error(GetLastError().0)) }
        } else {
            Ok(())
        }
    ...
    /// - `Ok(HANDLE)`: 驱动包句柄
    /// - `Err(...)`：失败则返回 FALSE，则包含 Win32 错误码或说明
    pub fn open_driver(&self, inf_path: &Path, arch: u16) -> Result<Handle, DrvstoreError> {
    ...
    /// - `Ok(())`:   返回成功
    /// - `Err(...)`：失败则返回包含 Win32 错误码的说明
    pub fn close_package(&self, handle: Handle) -> Result<(), DrvstoreError> {
        if !unsafe { (self.closePackage)(handle) } {
            unsafe { Err(DrvstoreError::Win32Error(GetLastError().0)) }
        } else {
            Ok(())
        }
    ...
    pub fn copy_driver(
        ...
        if result != 0 {
            Err(DrvstoreError::Win32Error(result))
        } else {
            Ok(())
        }
    }
    ...
    pub fn find_driver_package(
        &self,
        handle: Handle,
        inf_path: &Path,
        arch: u16,
    ) -> Option<(PathBuf, DriverPackageInfo)> {
        // 宽字符化输入
        ...
        unsafe {
            Some((
                ...
                DriverPackageInfo::from(info.assume_init()),
            ))
        }
    }
    ...
    pub fn get_version_info(
        &self,
        handle: Handle,
    ) -> Result<DriverPackageVersionInfo, DrvstoreError> {
        ...
        if !unsafe { (self.getVerinfo)(handle, &mut info as *mut _) } {
            return unsafe { Err(DrvstoreError::Win32Error(GetLastError().0)) };
        }
    ...
    pub fn import_driver_to_store(
        &self,
        handle: Handle,
        inf_path: &Path,
        arch: u32,
        flags: u32,
    ) -> Result<String, DrvstoreError> {
        ...
        if status != 0 {
            return Err(DrvstoreError::Win32Error(status));
        }
    ...
    pub fn delete_driver(
        ...
        let inf_w = to_wide(inf_path.as_os_str());
        let status = unsafe { (self.delete)(handle, inf_w.as_ptr(), flags) };
        if status == 0 {
            Ok(())
        } else {
            Err(DrvstoreError::Win32Error(status))
        }
    }
    ...
    /// - `Ok(())`: 返回成功
    /// - `Err(...)`：如果调用失败则返回包含 Win32 错误码的说明
    pub fn reflect_driver(&self, handle: Handle, flags: u32) -> Result<(), DrvstoreError> {
        let status = unsafe { (self.reflect)(handle, flags) };
        if status == 0 {
            Ok(())
        } else {
            Err(DrvstoreError::Win32Error(status))
    ...
    /// - `Ok(())`: 返回成功
    /// - `Err(...)`：如果调用失败则返回包含 Win32 错误码的说明
    pub fn unreflect_driver(
        &self,
        handle: Handle,
        inf_name: &str,
        flags: u32,
        filter_device_ids: Option<&str>,
    ) -> Result<(), DrvstoreError> {
        ...
        match status {
            0 => Ok(()),
            0x000000A1 => Err(DrvstoreError::Win32Error(status)),
            _ => Err(DrvstoreError::Win32Error(status)),
        }
    }
    ...
    /// - `Ok(())`: 返回成功
    /// - `Err(...)`：如果调用失败则返回包含 Win32 错误码的说明
    pub fn unreflect_critical_driver(
        &self,
        handle: Handle,
        inf_name: &str,
        flags: u32,
