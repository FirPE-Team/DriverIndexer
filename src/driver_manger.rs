...
use crate::utils::utils::{
    copy_dir, filetime_to_string, get_file_list, get_offline_system_arch, is_offline_system,
};
use crate::{command, TEMP_PATH};
use anyhow::Result;
...
impl DriverManger {
    pub fn new(system_drive: &Path) -> Result<Self> {
        ...
        if is_offline_system(system_drive).with_context(|| "check offline system failed")? {
            // 获取离线系统架构
            let offline = get_offline_system_arch(system_drive)
                .with_context(|| "get offline system arch failed")?;

            // 使用离线系统中的drvstore.dll
            let drvstore_path = match (env::consts::ARCH, offline) {
                // x86程序
                ("x86", 0x014c) => system_drive
                    .join("Windows")
                    .join("System32")
                    .join("drvstore.dll"),
                ("x86", 0x8664) => system_drive
                    .join("Windows")
                    .join("SysWOW64")
                    .join("drvstore.dll"),
                // x64程序
                ("x86_64", 0x8664) => system_drive
                    .join("Windows")
                    .join("System32")
                    .join("drvstore.dll"),
                // arm64程序
                ("aarch64", 0xAA64) => system_drive
                    .join("Windows")
                    .join("SysWOW64")
                    .join("drvstore.dll"),
                (_, _) => {
            ...
            // 在线系统
            Ok(DriverManger {
                driver_store: DriverStore::new(None)?,
            })
        }
    }
